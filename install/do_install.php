...
if (isset($_POST)) {
    ...
    $google_api_key = $_POST["google_api_key"] ? $_POST["google_api_key"] : "";
    $timezone = $_POST["timezone"];
    $purchase_code = $_POST["purchase_code"];

    //check required fields
    if (!($host && $dbuser && $dbname && $fullname && $email && $login_password && $purchase_code && $google_api_key && $timezone)) {
   ...
   if (empty($verification) && $verification->status = "success") {
    ...
    $sql = str_replace('admin_google_api_key', $google_api_key, $sql);
